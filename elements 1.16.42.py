...
import numpy as np
from scipy.special import sph_harm_y  # Updated import
import argparse
import logging
import sys
import os
...
# Ambisonic channels
element_channels = {
    'Silicon': [(1, 1), (1, -1)], 'Cadmium': [(1, 1)], 'Iron': [(1, -1)],
    'Francium': [(1, -1)], 'Oxygen': [(1, 0)], 'Copper': [(1, 0)],
    'Potassium-40': [(1, 0)], 'Carbon': [(2, 1)], 'Titanium': [(2, 1)],
    'Magnesium': [(2, -1)], 'Molybdenum': [(2, -1)], 'Hydrogen': [(2, 0)],
...
def spherical_harmonic(l, m, theta, element="Unknown"):
    ...
    if l < 0 or abs(m) > l:
        logging.error(f"Invalid spherical harmonic for {element}: l={l}, m={m}")
        return 0.0
    Y_lm = sph_harm_y(m, l, phi, theta).real  # Updated to sph_harm_y
    if not np.isfinite(Y_lm):
        logging.warning(f"Non-finite Y_lm for {element}: l={l}, m={m}, theta={theta:.2f}")
        return 0.0
    return Y_lm
...
def update_plot(x_shift, y_shift, z_scale, invert_w, active_elements, ax, fig, t=0, lissajous_ax=None):
    ...
    signal_scale = 10.0  # From previous fix for Lissajous visibility
    for elem in active_elements:
